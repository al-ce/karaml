...
def test_translate_event():
    ...
    assert mp.translate_template("not_an_event", "some command") == (
        "not_an_event", "some command")

    assert mp.translate_template("app", "Firefox") == (
    ...
    assert mp.translate_template("open", "https://github.com") == (
        "shell_command", "open https://github.com"
    )

    assert mp.translate_template("shell", "cd somedir/") == (
        "shell_command", "cd somedir/"
    )

    assert mp.translate_template("input", "en") == (
    ...
    assert mp.translate_template("input", "'{'some': 'dictionary'}'") == (
        "select_input_source", {"language": "'{'some': 'dictionary'}'"}
    )

    assert mp.translate_template("mouse", "x, 2000") == (
        "mouse_key", {"x": 2000}
    )

    assert mp.translate_template(
        "mouse",
        '''{
            "x": 2000,
            "y": 2000,
    ...
    assert mp.translate_template("mousePos", "1, 2") == (
        "software_function",
        {"set_mouse_cursor_position": {"x": 1, "y": 2}}
    )
    assert mp.translate_template("mousePos", "1, 2, 3") == (
        "software_function",
    ...
    with pytest.raises(SystemExit) as inv_mouse_pos_test:
        ...
        mp.translate_template("mousePos", "1")
    assert inv_mouse_pos_test.type == SystemExit

    with pytest.raises(SystemExit) as inv_mouse_pos_test:
        # Invalid mousePos arguments: max 3
        mp.translate_template("mousePos", "1, 2, 3, 4")
    assert inv_mouse_pos_test.type == SystemExit

    with pytest.raises(SystemExit) as inv_mouse_pos_test:
        # Invalid mousePos arguments: integers only
        mp.translate_template("mousePos", "one, two")
    assert inv_mouse_pos_test.type == SystemExit

    # Test for proper whitespace-stripping
    assert mp.translate_template("notify", " some_id ,some_message") == (
        "set_notification_message", {"id": "some_id", "text": "some_message"}
    )

    assert mp.translate_template("notifyOff", "some_id") == (
    ...
    assert mp.translate_template("softFunc", "some_arg") == (
        "software_function", "some_arg"
    )

    sticky_values = ["on", "off", "toggle"]
    for v in sticky_values:
        assert mp.translate_template("sticky", f"left_shift, {v}") == (
    ...
    with pytest.raises(SystemExit) as inv_sticky_mod_test:
        for m in unsupported_sticky_mods:
            ...
            mp.translate_template("sticky", f"{m}, on")
    ...
    with pytest.raises(SystemExit) as inv_sticky_val_test:
        mp.translate_template("sticky", "left_shift, some_value")
    assert inv_sticky_val_test.type == SystemExit

    assert mp.translate_template("var", "some_layer, 1") == (
        "set_variable", {"name": "some_layer", "value": 1}
    )

    with pytest.raises(SystemExit) as inv_var_value_type:
        mp.translate_template("var", "some_layer, not_a_number")
